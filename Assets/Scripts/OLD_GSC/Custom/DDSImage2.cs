		...
		private static void DecompressBlockDXT1(int x, int y, byte[] blockStorage, Bitmap image) {
			...
			for (int j = 0; j < 4; j++) {
				for (int i = 0; i < 4; i++) {
					...
					image.SetPixel(x + i, y + j, new UnityEngine.Color(finalColor.R/255f, finalColor.G / 255f, finalColor.B / 255f));
				}
			}
		}
		#endregion
...
#pragma warning restore CS0675
				...
				using (BinaryReader r = new BinaryReader(ms)) {
					for (int y = 0; y < h; y++) {
						for (int x = 0; x < w; x++) {
							...
							var col = Color.FromArgb(0x7F - red, 0x7F - green, blue);
                            res.SetPixel(x, y, new UnityEngine.Color(col.R / 255f, col.G / 255f, col.B / 255f));
						}
					}
				}
			}
		...
		private Bitmap readLinearImage(byte[] data, int w, int h) {
			...
			using (MemoryStream ms = new MemoryStream(data)) {
				using (BinaryReader r = new BinaryReader(ms)) {
					for (int y = 0; y < h; y++) {
						for (int x = 0; x < w; x++) {
							var col = Color.FromArgb(r.ReadInt32());
                            res.SetPixel(x, y, new UnityEngine.Color(col.R / 255f, col.G / 255f, col.B / 255f));
						}
					}
				}
		...
		private static Bitmap pixelsToBitmap(int[] pixels, int w, int h)
		{
			...
			for (int i = 0; i < h; i++)
			{
				for (int j = 0; j < w; j++)
				{
					var col = Color.FromArgb(pixels[j + i * w]);
                    bmp.SetPixel(j, i, new UnityEngine.Color(col.R / 255f, col.G / 255f, col.B / 255f));
				}
			}
			return bmp;
		}
