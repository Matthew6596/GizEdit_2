...
public class GizmoPickupLoader : PropertyLoader
{
    ...
    public override void Load(byte[] bytes, ref int index)
    {
        ...
        pickup.AddProperty(new StringFixLenProperty("Name", name, 8, "This is the name of the pickup, which can be used to reference this pickup in the .git file."));
        index += 8;
        ...
        if (version >= 3)
        {
            spawnGroup = bytes[index];
            index++;
        }
        if (ShouldAddProperty(version, v => v >= 3))
        {
            // ## Spawn Group ##
            pickup.AddProperty(new IntegerProperty("Spawn Group", spawnGroup, IntegerProperty.IntType.Byte, "This allows multiple pickups to spawn at the same time. If 0 it acts as normal. Otherwise, it will not appear and only spawn in once it or a pickup of the same spawn group is spawned by trigger."));
        }
