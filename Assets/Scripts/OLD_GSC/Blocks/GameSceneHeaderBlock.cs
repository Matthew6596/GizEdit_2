...
public class GameSceneHeaderBlock : DefaultFileBlock
{
    ...
    private void readTextureContents(TextureDescriptor descriptor)
    {
        ...
        byte[] imgBytes = SceneLoader.bytes.Skip(tmp).Take(descriptor.size).ToArray();
        ...
        int temp = 0;
        KUtility.Bitmap ddsTexture=null;
        string ext=extension;
        try
        {
            ddsTexture = KUtility2.DDSImage.ReadDDSFromGSC(imgBytes, ref temp, out ext);
            //Debug.Log("reading texture at" + temp);
        ...
        if (ddsTexture != null) 
        {
            ...
            Texture2D newTxtr = ddsTexture.txtr;
    ...
    private SceneMesh readMesh(int ptr)
    {
        ...
        meshObj.transform.SetParent(Temp_GSCFileLoader.parentTransform);
