...
public class GSCScene
{
    ...
    public static GSCScene LoadFromFile()
    {
        SFB.ExtensionFilter[] filters = new SFB.ExtensionFilter[] { 
            new("Game Scene", "gsc", "GSC", "Gsc") 
        };

        string[] paths = SFB.StandaloneFileBrowser.OpenFilePanel("LSW:TCS Scene File Dialog", "", filters, false);
        ...
        GSCScene scene = (Path.GetExtension(paths[0]).ToLower()) switch
        {
            ...
            _ => throw new FileLoadException("File extension must be .gsc")
        };
