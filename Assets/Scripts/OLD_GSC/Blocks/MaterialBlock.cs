...
class MaterialBlock : DefaultFileBlock
{
    public override void readFromFile(int blockSize, int blockId)
    {
        ...
        for (int i = 0; i < materialCount; i++)
        {
            ...
            byte surfaceIdx = SceneLoader.reader.ReadByte(); //uv index?
            ...
            SceneLoader.ReadLocation = ptr + 0xB4 + 0x1B4;
            int inputDefines = SceneLoader.reader.ReadInt32();
            int shaderDefines = SceneLoader.reader.ReadInt32();
            int uvsetCoords = SceneLoader.reader.ReadInt32();

            //if(uvsetCoords!=0)Debug.Log($"Mat{i} uv?: {surfaceIdx}, {uvsetCoords}");
            /*
